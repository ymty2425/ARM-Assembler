...
enum parse_opcode_enum parse_opcode(struct scan_token_st *tp) {
    ...
    if (parse_is_member(tp->value, parse_dp_ops)) {
        ...
        rv = OC_BX;
    } else if (parse_is_member(tp->value, parse_cmp_ops)) {
		rv = OC_CMP;
...
struct parse_node_st * parse_statement(struct parse_table_st *pt,
 									struct scan_table_st *st) {
    struct scan_token_st *tp0, *tp1;
	struct parse_node_st *np;

	tp0 = scan_table_get(st, 0);
	tp1 = scan_table_get(st, 1);

	if (tp0->id == TK_DIR && tp1->id == TK_IDENT) {
...
struct parse_node_st * parse_instruction(struct parse_table_st *pt,
    ...
    if ((opcode == OC_CMP) && (tp1->id == TK_IDENT) && 
        (tp2->id == TK_COMMA) && (tp3->id == TK_IDENT)) {
        /* Parse dp3 instructions: cmp reg, reg */
        ...
        /* Parse dp3 instructions: cmp reg, imm */
  
        np->stmt.inst.type = CMPI;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
        np->stmt.inst.cmpi.rn = parse_reg_to_int(tp1->value);
        np->stmt.inst.cmpi.imm = atoi(tp3->value);
        ...
        /* Parse dp3 instructions: dp reg, reg, imm */
  
        np->stmt.inst.type = DP3I;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
        np->stmt.inst.dp3i.rd = parse_reg_to_int(tp1->value);
        np->stmt.inst.dp3i.rn = parse_reg_to_int(tp3->value);
        np->stmt.inst.dp3i.imm = atoi(tp5->value);
        ...
        (tp2->id == TK_COMMA) && (tp3->id == TK_IDENT)) {
        /* Parse dp3 instructions: dp reg, reg */
  
        np->stmt.inst.type = DP2;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
        np->stmt.inst.dp2.rd = parse_reg_to_int(tp1->value);
        np->stmt.inst.dp2.rm = parse_reg_to_int(tp3->value);
        ...
        (tp2->id == TK_COMMA) && (tp3->id == TK_IMM)) {
        /* Parse dp3 instructions: dp reg, imm */
  
        np->stmt.inst.type = DP2I;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
        np->stmt.inst.dp2i.rd = parse_reg_to_int(tp1->value);
        np->stmt.inst.dp2i.imm = atoi(tp3->value);
        ...
        (tp2->id == TK_COMMA) && (tp3->id == TK_IDENT) && 
        (tp4->id == TK_COMMA) && (tp5->id == TK_IMM)) {
        /* Parse dp3 instructions: ls reg, reg, imm */
  
        np->stmt.inst.type = LSI;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
        np->stmt.inst.lsi.rd = parse_reg_to_int(tp1->value);
        np->stmt.inst.lsi.rm = parse_reg_to_int(tp3->value);
        np->stmt.inst.lsi.imm = atoi(tp5->value);
        ...
        /* Parse mem3 instructions: mem reg, [reg, imm] */
...
void parse_tree_print_inst(struct parse_node_st *np, int level) {
	...
	if (np->stmt.inst.type == CMP) {
        ...
        printf("cmpi (%s) %d %d\n", np->stmt.inst.name,
            np->stmt.inst.cmpi.rn, np->stmt.inst.cmpi.imm);
    } else if (np->stmt.inst.type == DP3) {
        printf("dp3 (%s) %d %d %d\n", np->stmt.inst.name,
            np->stmt.inst.dp3.rd, np->stmt.inst.dp3.rn, np->stmt.inst.dp3.rm);
    } else if (np->stmt.inst.type == DP3I) {
        printf("dp3 (%s) %d %d %d\n", np->stmt.inst.name,
            np->stmt.inst.dp3i.rd, np->stmt.inst.dp3i.rn, np->stmt.inst.dp3i.imm);
    } else if (np->stmt.inst.type == DP2) {
        printf("dp2 (%s) %d %d\n", np->stmt.inst.name,
            np->stmt.inst.dp2.rd, np->stmt.inst.dp2.rm);
    } else if (np->stmt.inst.type == DP2I) {
        printf("dp2i (%s) %d %d\n", np->stmt.inst.name,
            np->stmt.inst.dp2i.rd, np->stmt.inst.dp2i.imm);
    } else if (np->stmt.inst.type == LSI) {
        printf("lsi (%s) %d %d %d\n", np->stmt.inst.name,
            np->stmt.inst.lsi.rd, np->stmt.inst.lsi.rm, np->stmt.inst.lsi.imm);
        ...
        printf("memi (%s) %d %d %d\n", np->stmt.inst.name,
            np->stmt.inst.memi.rd, np->stmt.inst.memi.rn, np->stmt.inst.memi.imm);
    } else if (np->stmt.inst.type == B) {
        printf("b (%s) %s\n", np->stmt.inst.name, np->stmt.inst.b.label);
    } else if (np->stmt.inst.type == BX) {
        printf("bx (%s) %d\n", np->stmt.inst.name, np->stmt.inst.bx.rn);
    }
}

void parse_tree_print_dir(struct parse_node_st *np, int level) {
	parse_tree_print_indent(level);
...
void parse_tree_print_stmt(struct parse_node_st *np, int level) {
    parse_tree_print_indent(level);
	if (np->type == DIR) {
		parse_tree_print_dir(np, level+1);
